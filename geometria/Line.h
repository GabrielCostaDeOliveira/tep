...
struct Line{
    ...
    Line (const Point<T> &p,const Point<T> &q): a(p.y - q.y), b(q.x - p.x), c(p.x*p.q - q.x*p.y) {}
    ...
    std::pair <int, Point<T>> intersection (const Line<T>& r) const;
...
template <typename T>
std::pair <int, Point<T>> Line<T>::intersection(const Line<T>& r) const{
    auto det = a*r.b - b*r.a;
...
template <typename T>
double Line<T>::angle (const Point<T> &p,const Point<T> &q,const Point<T> &r,const Point<T> &s){
    auto ux = p.x - q.x;
    auto uy = p.y - q.y;
